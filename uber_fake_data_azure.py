...
# number of records for vehicle data and driver data
num_records_2022 = 1000
num_records_2021 = 900
num_records_2020 = 850

# number of records for passenger data
pass_2020 = 1000
pass_2021 = 1200
pass_2022 = 1500
...
# function to generate vehicle dataframe
...
# function to generate driver dataframe
def generate_and_upload_driver_data(year, num_records , v_ids , connection_string):
    ...
    for i, d_id in enumerate(d_ids):
        ...
        if d_gender == 'M':
            d_name = fake.first_name_male()
        else:
            d_name = fake.first_name_female()
...
def generate_and_upload_passenger_data(year , pass_records , connection_string):
    ...
    for pass_id in pass_ids:
        ...
        record = {'pass_id': pass_id , 'pass_name' : pass_name , 'pass_ph_no' : pass_ph_no ,
                ...
                'pass_gender': pass_gender , 'device_os':device_os , 'wallet_attached': wallet_attached  }
...
def generate_and_upload_ride_data(year , ride_records , pass_ids , d_ids , c_ids , coupon_df , coupon_bridge_table , driver_df , connection_string):
    ...
    for r_id in r_ids:
        ...
        record = {'r_id':r_id , 'r_pass_id': r_pass_id , 'r_d_id' : r_d_id , 
                ...
                'total_dist_covered_km' : total_dist_covered_km , 'r_drop_time':r_drop_time ,'ride_fare' : ride_fare , 'ride_tip': ride_tip ,'ride_cancelled_by_whom': ride_cancelled_by_whom, 
...
dir_path =  '/etc/dataos/config/'   
file_name = 'azureCred.conf'
fields_to_extract = ["AZURE_CONNECTION_STRING",'AZURE_CONTAINER_NAME']
...
for field in fields_to_extract:
    ...
    field_values[field] = json_data.get(field)

def upload_to_azure(file_prefix, year, connection_string):
    # container_name = os.popen('cat /etc/dataos/secret/CONTAINER_NAME').read() 
    container_name = decode(field_values['AZURE_CONTAINER_NAME'])
 
    folder_name = "uber_data1"
    subfolder_name = None
